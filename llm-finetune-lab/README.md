...
Open-source, plug-and-play framework for finetuning an open LLM on real business domains. Each domain has its own folder under `domains/`, with a verified dataset, an honest eval, and real (not cherry-picked) results.
...
Each domain pairs a verified public dataset with an eval appropriate to that domain's actual task.

| Domain | Folder | Dataset (license) | Primary eval |
|---|---|---|---|
| Code | `domains/code/` | glaive-code-assistant-v3 (Apache-2.0) | HumanEval+ / MBPP+ pass@1 |
| Finance | `domains/finance/` | Finance-Instruct-500k (Apache-2.0) | held-out perplexity |
| Healthcare | `domains/healthcare/` | medical-o1-reasoning-SFT (Apache-2.0) | MedQA-USMLE accuracy |
| Legal | `domains/legal/` | CUAD clause classification (CC-BY-4.0) | clause-label accuracy |
| Customer Support | `domains/customer_support/` | Bitext support (CDLA-Sharing-1.0) | held-out perplexity |
| Marketing | `domains/marketing/` | marketing-instruct-13k (Apache-2.0) | held-out perplexity |
...
That runs download → dedup → decontaminate → holdout split locally, no GPU needed. Then open `notebooks/quickstart_colab.ipynb` in Google Colab, set `DOMAIN` in its first code cell, (Runtime → Change runtime type → GPU) and run it top to bottom: it trains the adapter, evals baseline and finetuned with the same harness, and writes `domains/code/results/`.
